...
public class HuNanRegActivity extends RxActivity implements IFaceView, IIDCardView {
...
//    }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        disposableTimer = RxTextView.textChanges(tv_timer)
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe((charSequence) -> fp.PreviewCease(() -> HuNanRegActivity.this.finish()));
    }
...
//        fp.PreviewCease(() -> HuNanRegActivity.this.finish());
    ...
    public void onsetCardImg(Bitmap bmp) {
        ...
        if (bmp != null) {
            try {
                mdaosession.queryRaw(Employer.class, "where CARD_ID = '" + global_cardInfo.cardId().toUpperCase() +
                        "'").get(0);
                //            idp.stopReadCard();
                ...
                RetrofitGenerator.getHnmbyApi().queryPersonInfo("queryPersion", config.getString("key"), global_cardInfo.cardId())
    ...
    private void recentPic() {
        ...
        RetrofitGenerator.getHnmbyApi()
                .recentPic("recentPic", config.getString("key"), global_cardInfo.cardId())
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
