...
public class WYYService extends Service implements ISwitchView {
...
//                e.onNext(ApkUtils.sendDownload(""));
//            }
//        })
//                .subscribeOn(Schedulers.io())
//                .unsubscribeOn(Schedulers.io())
...
//                .subscribe((o) -> {
//                    if (o.equals("true")) {
//                        if (SignUtils.checkMd5(UpdateConstant.ORIGINAL_APK_PATH, ORIGINAL_APK_PATH)) {
//                            int patchResult = PatchUtils.patch(UpdateConstant.ORIGINAL_APK_PATH, UpdateConstant.NEW_APK_PATH, UpdateConstant.PATCH_PATH);
//                            if (patchResult == 0) {
//                                if (SignUtils.checkMd5(UpdateConstant.NEW_APK_PATH, New_APK_MD5)) {
//                                    ApkUtils.installApk(AppInit.getContext(), UpdateConstant.NEW_APK_PATH);
//                                } else {
//                                    ToastUtils.showLong("新合成的apk与设定MD5值不一");
//                                }
//                            } else {
//                                ToastUtils.showLong("apk合成失败");
//                            }
//                        } else {
//                            ToastUtils.showLong("旧有的MDK值pk与设定MD5值不一");
//                        }
//                    }
//                });
//    }
...
//                    FingerPrintPresenter.getInstance().fpRemoveAll();
    ...
    private void equipment_sync(String old_devid) {
        ...
        try {
            ...
            Log.e("reboot_equ_Exception", e.toString());
        }
                                            ...
                                            .subscribe((l) -> {
                                                try {
                                                    ...
                                                    for (int i = 0; i < jsonArray.length(); i++) {
                                                        ...
                                                        mdaoSession.insert(employer);
                                                    }
                                                    List<FingerprintUser> fingerprintUsers = mdaoSession.loadAll(FingerprintUser.class);
                                                    for (FingerprintUser user : fingerprintUsers) {
                                                        try {
                                                            mdaoSession.queryRaw(Employer.class, "where CARD_ID = '" + user.getCardId() + "'").get(0);
                                                        } catch (IndexOutOfBoundsException e) {
                                                            mdaoSession.delete(user);
                                                            FingerPrintPresenter.getInstance().fpRemoveTmpl(user.getFingerprintId());
